...
(() => {
  ...
  const zombieArt = new Image();
  playerArt.src = 'assets/survivor-v2.png';
  zombieArt.src = 'assets/zombies-v2.png';

  const player = { x: .5, y: .84, targetX: .5, radius: 25, invuln: 0, muzzle: 0, move: 0 };

  const enemyTypes = {
    runner: { hp: 1, speed: 42, radius: 17, color: '#ff668e', accent: '#ffbfcc', score: 100, damage: 8, sprite: 0, spriteSize: .13 },
    striker: { hp: 2, speed: 31, radius: 21, color: '#a28cff', accent: '#e3dcff', score: 180, damage: 13, sprite: 2, spriteSize: .145 },
    brute: { hp: 5, speed: 18, radius: 31, color: '#ffbd62', accent: '#fff0bd', score: 420, damage: 25, sprite: 1, spriteSize: .19 },
    crawler: { hp: 3, speed: 38, radius: 22, color: '#78e0c3', accent: '#d3fff3', score: 260, damage: 17, sprite: 3, spriteSize: .16 },
  };
  ...
  function spawnEnemy() {
    ...
    else if (wave >= 4 && roll > .68) kind = 'crawler';
  ...
  function drawPlayer() {
    ...
    const spriteSize = Math.min(190, Math.max(94, width * .27));
    ctx.save();
    ctx.globalAlpha = player.invuln > 0 && Math.floor(player.invuln * 20) % 2 === 0 ? .45 : 1;
    ctx.fillStyle = 'rgba(0,0,0,.38)';
    ctx.beginPath(); ctx.ellipse(x, y + spriteSize * .32, spriteSize * .26, spriteSize * .1, 0, 0, TAU); ctx.fill();
    const glow = ctx.createRadialGradient(x, y, 4, x, y, spriteSize * .72);
    glow.addColorStop(0, 'rgba(89,229,255,.24)'); glow.addColorStop(1, 'rgba(89,229,255,0)');
    ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(x, y, spriteSize * .72, 0, TAU); ctx.fill();
    ctx.translate(x, y);
    ctx.rotate(angle + Math.PI / 4);
    if (playerArt.complete && playerArt.naturalWidth) {
      ctx.shadowBlur = player.muzzle > 0 ? 24 : 12; ctx.shadowColor = '#59e5ff';
      ctx.drawImage(playerArt, -spriteSize / 2, -spriteSize / 2, spriteSize, spriteSize);
      ctx.shadowBlur = 0;
      ctx.fillStyle = player.muzzle > 0 ? '#fff8ba' : '#ff5b8f';
      ctx.shadowBlur = player.muzzle > 0 ? 28 : 11; ctx.shadowColor = '#ff5b8f';
      ctx.beginPath(); ctx.arc(spriteSize * .35, -spriteSize * .33, player.muzzle > 0 ? 9 : 4, 0, TAU); ctx.fill();
      if (player.muzzle > 0) { ctx.fillStyle = 'rgba(255,218,126,.42)'; ctx.beginPath(); ctx.moveTo(spriteSize * .28, -spriteSize * .32); ctx.lineTo(spriteSize * .03, -spriteSize * .47); ctx.lineTo(spriteSize * .18, -spriteSize * .17); ctx.closePath(); ctx.fill(); }
    } else {
      ctx.fillStyle = '#59e5ff'; ctx.beginPath(); ctx.arc(0, 0, spriteSize * .2, 0, TAU); ctx.fill();
      ctx.fillStyle = '#ff5b8f'; ctx.fillRect(spriteSize * .12, -5, spriteSize * .28, 10);
    }
    ctx.restore();
  }
  ...
  function drawEnemies() {
    for (const enemy of enemies) {
      const type = enemyTypes[enemy.kind];
      const x = screenX(enemy.x), y = screenY(enemy.y) + Math.sin(enemy.phase * 2) * 2;
      const spriteSize = Math.min(height * .18, Math.max(52, width * type.spriteSize));
      const cell = zombieArt.naturalWidth ? zombieArt.naturalWidth / 2 : 0;
      ctx.save();
      ctx.globalAlpha = enemy.flash > 0 ? .72 : 1;
      ctx.fillStyle = 'rgba(0,0,0,.4)'; ctx.beginPath(); ctx.ellipse(x, y + spriteSize * .31, spriteSize * .27, spriteSize * .095, 0, 0, TAU); ctx.fill();
      const glow = ctx.createRadialGradient(x, y, 0, x, y, spriteSize * .7); glow.addColorStop(0, `${type.color}45`); glow.addColorStop(1, `${type.color}00`); ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(x, y, spriteSize * .7, 0, TAU); ctx.fill();
      ctx.translate(x, y); ctx.rotate(Math.sin(enemy.phase * .7) * .04);
      if (zombieArt.complete && cell) {
        const sx = (type.sprite % 2) * cell; const sy = Math.floor(type.sprite / 2) * cell;
        ctx.shadowBlur = enemy.flash > 0 ? 20 : 8; ctx.shadowColor = type.color;
        ctx.drawImage(zombieArt, sx, sy, cell, cell, -spriteSize / 2, -spriteSize / 2, spriteSize, spriteSize);
      } else {
        ctx.fillStyle = type.color; ctx.beginPath(); ctx.arc(0, 0, spriteSize * .25, 0, TAU); ctx.fill();
      }
      ctx.restore();
      if (enemy.maxHp > 1) { const barW = spriteSize * .55; const barY = y - spriteSize * .42 - 7; ctx.fillStyle = 'rgba(0,0,0,.7)'; ctx.fillRect(x - barW / 2, barY, barW, 4); ctx.fillStyle = type.color; ctx.fillRect(x - barW / 2, barY, barW * enemy.hp / enemy.maxHp, 4); }
    }
  }
